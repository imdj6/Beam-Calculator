...
import DoublyDiagram from './DoublyDiagram';

const DoublyTable = (props) => {
    ...
    function area(d) {
        return parseFloat((Math.PI*d*d)/4);
    }
    ...
    return (
        ...
        <Text className={`mt-5  text-center w-36 ml-16 ${props.astfordouble>totalareatension?"bg-red-600":"bg-green-600"}`}>{totalareatension}</Text>
        <Text className={`mt-5 text-center w-36 ml-6  ${props.asc>totalcompressiontension?"bg-red-600":"bg-green-600"}`}>{totalcompressiontension}</Text>
        </View>
        <DoublyDiagram total8mm={total8mm} total10mm={total10mm} 
        total12mm={total12mm} total16mm={total16mm} total20mm={total20mm}
        total25mm={total25mm}
        depth={props.depth} breadth={props.br}/>
        </>
    )
}

export default DoublyTable